...
class mfcc:
    ...
    def framing(self, signal, frame_length, frame_step):
        ...
        # indices for framing
        indices = np.array(np.tile(np.arange(0, frame_length), (number_of_frames, 1)) + np.transpose(
            np.tile(np.arange(0, number_of_frames * frame_step, frame_step), (frame_length, 1))), dtype=np.int32)
    ...
    def calculate_mel_filter_banks(self, num_filters=20, nfft=512, sample_rate=16000, lower_frequency=133.33,
